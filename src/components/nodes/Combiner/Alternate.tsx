  ...
  ({ id, data }) => {
    ...
    React.useEffect(() => {
      ...
      if (TargetConnections?.length > 0) {
        ...
        valid = targetConnections.every((target) => target?.id);
        sourcePlaylists = valid ? (targetConnections as Playlist[]) : [];
        invalidNodesCount = targetConnections.filter(
          (target) => !target?.id,
        ).length;
      }

      // get total of all playlists
      const total = sourcePlaylists.reduce(
          ...
          <div className="flex flex-col text-sm font-medium">
            {TargetConnections?.length === 0 ? (
              ...
              <AlertComponent
                variant="destructive"
                title="Error"
                description={`${state.invalidNodesCount} nodes do not have a playlist component connected to them`}
              />
            )}
          </div>

          <div className="flex flex-col gap-2">
            {state.sourcePlaylists?.map((playlist) => (
